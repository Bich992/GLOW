...
import { computeBoostPriority, computeDeltaSeconds } from './economy';
import { debitWallet } from './wallet';
import { createNotification } from './notifications';
import { applyBoostFuel } from './engagement';
...
export async function publishPost(
  ...
  const bornDuringWave = activeWave !== null;
  const initialDurationMs = bornDuringWave ? 60 * 60 * 1000 : 30 * 60 * 1000;
  ...
  return await prisma.$transaction(async (tx) => {
    const post = await tx.post.create({
      data: {
        ...
        publishedAt: now,
        born_at: now,
        born_during_wave: bornDuringWave,
        expiresAt,
        boostPriority: 1.0,
...
export async function extendPost(userId: string, postId: string): Promise<{ newExpiresAt: Date }> {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    const extensionsTodayByUser = await tx.postExtension.count({
      where: { postId, userId, createdAt: { gte: today } },
    });
    ...
    await tx.postExtension.create({
      data: { postId, userId, deltaSeconds: actualDelta, extensionIndex: userExtensionCount },
    });
...
export async function boostPost(
  ...
  // Apply glow engine fuel from boost amount
  await applyBoostFuel(postId, actualAmount).catch(
    (e: unknown) => logger.warn('applyBoostFuel failed', { postId, err: String(e) })
  );
